...
/**
 * 검증기가 잡은 오류를 그대로 되먹인다.
 *
 * 툴콜이 아니라 그냥 다음 user 턴이다. 프로토콜을 안 쓰므로 어느 모델에나
 * 통하고, 모델이 아는 것(카탈로그)과 우리가 아는 것(진짜 파서)이 한 번 더
 * 만난다. "원래 요구는 그대로" 를 못 박는 게 중요하다 — 안 그러면 오류를
 * 고치면서 사용자가 시킨 것까지 바꾼다.
 */
export function repairText(command, errors) {
  return `방금 준 명령어를 검증기에 돌렸더니 오류가 있다.
...
이 오류만 고쳐서 다시 내라. 원래 요구는 그대로 두고, 오류와 무관한 곳은
건드리지 마라. 답의 형식은 아까와 같다.`;
}
