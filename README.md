...
191개 옵션을 평평한 목록이나 폼으로 늘어놓으면 **"이 실행이 어떤 순서로 무엇을
거치는지"**가 지워진다. `--cookies-from-browser` 와 `--embed-subs` 가 실행
생애주기에서 완전히 다른 시점에 작동한다는 사실이 보이지 않는다.
...
"이번엔 빼고 돌려보기"가 된다. 끊긴 노드는 흐려지고 와이어는 점선이 되며,
하단 노트가 무엇이 빠졌는지 알려 준다.
...
결과만 보면 이 어긋남이 안 보이므로, e2e 가 **화면에 보이는 값**을 직접 읽는다.
...
`app.js` 가 갖는 것은 캔버스와 포인터와 배선뿐이다.

**HTML 문자열을 조립하지 않는다.** `innerHTML` 을 쓰는 곳은 넷인데 전부 데이터가
안 들어가는 자리다 — 레이어 비우기, 고정된 `<title>`, 그리고 레지스트리가 일부러 `<b>` 를
넣어 둔 안내 문구 둘. 사용자 문자열은 전부 템플릿의 값 자리로만 들어가고 lit 이
이스케이프한다 — 손으로 이스케이프하는 곳은 없다. 단위 테스트가 이 불변식을
...
만드는 중인 음절도 브라우저가 알아서 지킨다 — 포커스를 담았다 되돌리는 기구나
조합 중인 노드를 건너뛰는 예외는 없다. 값을 바꾸면 `render()` 한 번, 그게 전부다.
...
내보내고, 테스트는 내부 함수 이름을 모른다. 내부를 어떻게 바꿔도 테스트가
리팩터링의 브레이크로 남게 하려는 것이다 — 시그니처가 바뀌면 손잡이 안에서 흡수한다.
