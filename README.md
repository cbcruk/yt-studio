...
여기 있는 셋은 **언젠가 고칠 것**들이다. 아래는 성격이 다르다 — 고칠 것이 아니라
고른 것이다.

## 안 하기로 한 것

**실제로 돌려 보지 않는다.** 이 URL 에 그 포맷이 정말 있는지는 yt-dlp 를 실행해야
안다. 그건 이 도구가 못 하는 게 아니라 **안 하기로 한 것**이다.

```
검증기가 파는 것 = 어제와 오늘 같은 답
실행을 넣으면 = 네트워크 · 진행률 · 스트리밍 · 프로세스 오류
             = 첫 번째로 깨지는 것이 그 성질
```

이 저장소는 입력을 세 번 바꿨는데(노드 그래프 → 프롬프트 → 코드) 세 번 다
살아남은 게 **로컬에서, 결정적으로, 네트워크 없이 도는 검사**였다. 그게 이 도구가
LLM 에게 물어보는 것과 다른 이유이기도 하다 — 모델도 그럴듯한 답을 주지만 어제와
오늘이 같다는 보장이 없다.

실행이 필요하면 argv 를 받아 직접 띄우면 된다. 한 줄이다.
...
그다음부터 전부 무시하게 된다. 7번째 규칙은 누군가 실제로 틀렸을 때 온다.
