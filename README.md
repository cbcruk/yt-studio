...
- **`-f` 파서가 yt-dlp 를 다 따라가지 못한다.** 필터를 그룹에 바로 붙인
  표현식(`(mp4,webm)[height<480]` — yt-dlp 문서에 나오는 예다)을 우리 파서가
  거부한다. 검증기가 같은 파서를 쓰므로 이건 **거짓 오류**다. 그래프 쪽
  제약이기만 한 게 아니라 검증기가 멀쩡한 명령어를 틀렸다고 하는 자리다.
